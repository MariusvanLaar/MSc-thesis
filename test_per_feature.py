...
def train(model, optim, data_filename, batch_size, epochs, val_batch_size, kfolds=10, seed=5, **kwargs):
    ...
    torch.manual_seed(seed+987654321)
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(dataclass.data)):
        # print(fold)
        torch.manual_seed(seed+fold+987654321)
        np.random.seed(seed+fold+987654321)
...
if __name__ == "__main__":
    ...
    results = []
    dataset = "spectf"
    features = datasets.all_datasets[dataset]().num_features()
    print(features)
    ...
    for feat in range(features):
        ...
        R = train(model, optim, dataset, batch_size, epochs, batch_size*5, kfolds, seed,
                  n_blocks=n_blocks, n_qubits=n_qubits, index=feat)
        results.append(R)
    ...
    pickling_on = open(dataset+"_single_qubit_classifier.pkl","wb")
