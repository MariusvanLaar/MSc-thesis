        ...
        for LR in lrs:
            ...
            if len(files) != 5:
                ...
                for i, f in enumerate(files):
                    ...
                    if OPT != "spsa":
                        Lv[i] = run_dict["validation_loss"][::10]
                    else:
                        Lv[i,-1] = run_dict["validation_loss"][0]
                                        
                median_v_loss.append(np.median(Lv[:,-1]))
                std_vloss.append(mad(median_v_loss))
            
        if OPT != "lbfgs":
            ax1.errorbar(lrs, median_v_loss,  label=OPT, alpha=0.75)
            ax2.plot(lrs, std_vloss, label=OPT, alpha=0.8)
        else:
            ax1.errorbar(lrs[:-2], median_v_loss, label=OPT, alpha=0.75)
            ax2.plot(lrs[:-2], std_vloss, label=OPT, alpha=0.8)
    ax1.set_title(MOD)
    ax1.set_ylim(0.0, 0.9)
#    ax2.set_yscale('log')
    ax2.set_ylim(1e-4, 0.2)
...
#                     pickle_open = open(f, 'rb')
#                     run_dict = pickle.load(pickle_open)
#                     accs[i] = run_dict["validation_accuracy"][::10]
